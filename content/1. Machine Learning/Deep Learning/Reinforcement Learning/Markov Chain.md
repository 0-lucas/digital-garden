...
![[Pasted image 20250827201008.png]]
...
Given also a [[Vector|vector]] of *probabilities of occurrence of each state* $\Gamma = \{\gamma_{1},\gamma_{2},\dots,\gamma_{n}\}$, where $\gamma_{i}=P(S_{o}=q_{i})$;
and a [[Matrices|matrix]] of *transition probabilities (probabilities of change from a state to another)* $A = \{a_{ij}\},i=[1\dots n], j=[1\dots n]$ and $a_{ij}=P(S_{t}=q_{j}|S_{t-1}=q_{i})$, one can begin to model a *Markov Chain*.
