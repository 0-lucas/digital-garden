...
Which equals the **probability of going to state $S_{t+1}$ given $S_{t}$**  and the **probability of going to state $S_{t+1}$ given all past states**. In short, the information of previous states does not contribute to predict the future state: only the present state is significant.

>[!tip] Not all environments and processes follows this property. 
